    ...
    def lazy_load(self) -> Iterator[Document]:
        ...
        try:
            ...
            while True:
                ...
                res_list = db.query(Res).filter(Res.no > last_no).order_by(Res.no).limit(self.batch_size).all()
        ...
        while position < len(all_docs):
            ...
            if window_docs:
                ...
                parent_metadata = {
                    ...
                    "res_details": json.dumps(res_details, ensure_ascii=False),  # JSON文字列として保存
                    "source": f"window_{window_count}_no{window_docs[0].metadata['no']}-{window_docs[-1].metadata['no']}"
                }
