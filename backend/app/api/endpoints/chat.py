...
from app.rag.schemas import AskRequest, StreamToken
from app.rag.chain import ask_with_streaming, ask_question
    ...
    try:
        ...
        async for token in ask_with_streaming(question):
            ...
            if citation_processing and "[CITATION_END]" in buffer:
                ...
                # 引用情報を抽出
                citation_json = citation_content.replace("[CITATION_START]", "").replace("[CITATION_END]", "")
                try:
